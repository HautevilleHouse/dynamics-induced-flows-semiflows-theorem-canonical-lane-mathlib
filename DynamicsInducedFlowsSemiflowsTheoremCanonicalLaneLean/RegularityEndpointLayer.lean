...
def analyticAdmittedObject : AdmittedTheoremObject := {
  ...
  localWitness := "Dynamics induced flows semiflows analytic certificate with flow-semiflow envelope, compactness-rigidity gate, and regularity endpoint."
...
def analyticAdmissibleClass : AdmissibleClass := {
  ...
  endpointSatisfied := DynamicsEquationClosed primitiveInducedFlow
  remainderRecorded := formalizationCertificate.theoremBoundaryOpen = true
  gateWitness := Or.inl primitive_dynamics_equation_closed_checked
}
